...
export const run = async (deviceId, filteredData, inputs, outputKey, options, db, lastRecord, previousValue) => {
    ...
    if (typeof temp !== "number" || typeof pressure !== "number") {
        log.debug(`${LOG_TAG} Missing or invalid inputs: temp=${temp}, pressure=${pressure}`, logLevel);
        return undefined;
    }
    ...
    log.debug(`${LOG_TAG} Calculated ${p0.toFixed(2)} hPa from P=${pressure}, T=${temp}, Alt=${altitude}`, logLevel);
